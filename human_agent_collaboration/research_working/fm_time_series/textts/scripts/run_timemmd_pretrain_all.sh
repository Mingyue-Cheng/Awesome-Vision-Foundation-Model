...
BATCH_OUTPUT_DIR="${BATCH_OUTPUT_DIR:-$PROJECT_ROOT/outputs/timemmd_pretrain_all}"
...
printf 'Running batch pretrain for %s domains\n' "${#DOMAIN_LIST[@]}"
...
for domain in "${DOMAIN_LIST[@]}"; do
  ...
  printf '\n[%s] pretrain -> %s\n' "$domain" "$output_dir"
  if TIMEMMD_ROOT="$TIMEMMD_ROOT" OUTPUT_DIR="$output_dir" "$TEXTTS_DIR/scripts/run_timemmd_pretrain.sh" "$domain" >"$log_path" 2>&1; then
...
printf '\nBatch pretrain summary written to %s\n' "$SUMMARY_FILE"
