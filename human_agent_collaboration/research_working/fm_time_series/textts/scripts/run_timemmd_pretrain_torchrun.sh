...
printf 'Running TextTS torchrun pretrain with domains=%s nproc=%s backend=%s device=%s output=%s\n' "${DOMAINS:-$DOMAIN}" "$NPROC_PER_NODE" "$DISTRIBUTED_BACKEND" "$DEVICE" "$OUTPUT_DIR"
"${CMD[@]}"
